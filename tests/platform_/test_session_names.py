...
# --------------------------------------------------------------------------
# Project-key fallback — the dual-write that makes rename survive Claude
# Code rotating sessionId for the same pid (/clear, /resume, /compact).
# --------------------------------------------------------------------------

class TestProjectKeyFallback:
    def test_set_writes_both_keys_when_project_path_given(self, tmp_path_patched):
        # Both keys land in the file — the per-uuid one for an exact
        # match, the per-project one as the fallback.
        session_names.set_session_name(
            "uuid-1", "feature work", project_path="/home/me/proj-a",
        )
        data = json.loads(tmp_path_patched.read_text(encoding="utf-8"))
        assert data == {
            "uuid-1": "feature work",
            ":project:/home/me/proj-a": "feature work",
        }

    def test_get_falls_back_to_project_when_uuid_misses(self, tmp_path_patched):
        # Simulate Claude Code's /clear: rename was saved under uuid-A,
        # then sessionId rotated to uuid-B. The lookup with uuid-B alone
        # would miss; with project_path it picks up the project entry.
        session_names.set_session_name(
            "uuid-A", "named", project_path="/home/me/proj",
        )
        # Lookup with the rotated uuid + same project → still resolves.
        assert session_names.get_session_name(
            "uuid-B", project_path="/home/me/proj",
        ) == "named"

    def test_uuid_match_wins_over_project_match(self, tmp_path_patched):
        # If a session has its own override AND the project also has
        # one (e.g. user renamed the project, then renamed one specific
        # session in it), the per-session name should win.
        session_names.set_session_name(
            "uuid-1", "project name", project_path="/home/me/proj",
        )
        # Now overwrite just the uuid with a per-session name; project
        # entry stays at "project name".
        names = json.loads(tmp_path_patched.read_text(encoding="utf-8"))
        names["uuid-1"] = "session name"
        tmp_path_patched.write_text(json.dumps(names), encoding="utf-8")
        assert session_names.get_session_name(
            "uuid-1", project_path="/home/me/proj",
        ) == "session name"

    def test_empty_string_deletes_both_keys(self, tmp_path_patched):
        # Restore-default gesture clears the per-session AND the
        # per-project entry — half-clears would leave the project
        # name as a confusing zombie that re-applies to any future
        # session in the same dir.
        session_names.set_session_name(
            "uuid-1", "name", project_path="/home/me/proj",
        )
        session_names.set_session_name(
            "uuid-1", "", project_path="/home/me/proj",
        )
    ...
    def test_get_with_no_project_path_only_checks_uuid(self, tmp_path_patched):
        # Backward-compatible call shape (just uuid) must still work
        # for tests / callers that don't have a project_path handy.
        session_names.set_session_name("uuid-1", "named")
        assert session_names.get_session_name("uuid-1") == "named"
        assert session_names.get_session_name("missing") is None

    def test_set_without_project_path_only_writes_uuid_key(self, tmp_path_patched):
        # Skipping project_path means single-key write — same as the
        # original API behaviour. Lets us keep platform tests focused
        # on per-uuid behaviour without forcing a project arg.
        session_names.set_session_name("uuid-1", "named")
        data = json.loads(tmp_path_patched.read_text(encoding="utf-8"))
        assert data == {"uuid-1": "named"}

    def test_gc_keeps_project_keys_intact(self, tmp_path_patched):
        # Even with no known uuids that match a project entry, the
        # project entry stays — projects don't get garbage-collected
        # by uuid liveness.
        session_names.set_session_name(
            "uuid-A", "name", project_path="/home/me/proj",
        )
        # Simulate uuid-A being unknown (session deleted) but the
        # project still relevant. After gc, the per-uuid key is gone
        # but the per-project one carries the rename to the next
        # session in /home/me/proj.
        session_names.gc_session_names({"some-other-uuid"})
        assert session_names.get_session_name("uuid-A") is None
        assert session_names.get_session_name(
            "any-new-uuid", project_path="/home/me/proj",
        ) == "name"
