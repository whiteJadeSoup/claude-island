...
UUID (the JSONL filename stem) so it survives restarts and is stable
across pid reassignments.

Storage: ``~/.claude-island/session_names.json``::

    {
      "<session_uuid>": "<custom display name>",
      ...
    }
...
def get_session_name(uuid: str) -> str | None:
    ...
    Returned strings are guaranteed non-empty (an empty value is
    treated as "no override" so :func:`set_session_name` can use the
    empty string as the "restore default" sentinel without leaving a
    misleading empty entry in the file).
    """
    if not uuid:
        return None
    name = _read().get(uuid)
...
def delete_session_name(uuid: str) -> None:
    ...
    :func:`set_session_name` with an empty value — kept as a separate
    function because callers wanting to restore the default name read
    more clearly as ``delete_session_name(...)`` than as
...
def gc_session_names(known_uuids: set[str]) -> None:
    ...
    transcript. Called at app startup once the JSONL parser has indexed
    every transcript on disk; without this, deleted sessions would
    accumulate forever in the override file.
