...
class TestDetailPopupOriginalNameSubtitle:
    """The italic subtitle line under the title surfaces what the
    session would have been called WITHOUT the user's rename. Falls
    back to original_name (auto-detected name) when ai_title is
    absent — without this, sessions without an ai_title show no hint
    of what they used to be called after a rename."""

    @staticmethod
    def _find_subtitle_label(popup, expected_text: str):
        """Walk the popup's QLabels looking for one with the expected
        text. Cheaper than testing the layout structure and decouples
        the test from header-row index changes."""
        from PySide6.QtWidgets import QLabel
        for lbl in popup.findChildren(QLabel):
            if lbl.text() == expected_text:
                return lbl
        return None

    def test_subtitle_shows_ai_title_when_renamed(self, qtbot):
        # Classic case: AI generated a title, user renamed → italic
        # shows the AI title underneath.
        from claude_island.ui.expanded_window import SessionDetailPopup
        s = _session(1, "/proj")
        ...
        popup = SessionDetailPopup(details, s)
        qtbot.addWidget(popup)
        assert self._find_subtitle_label(
            popup, "Learn Python and TypeScript basics",
        ) is not None

    def test_subtitle_falls_back_to_original_name_when_no_ai_title(self, qtbot):
        # The bug fix: session has no ai_title (Claude Code never set
        # one) but DOES have a state.name that the user renamed away
        # from. Italic should show the original state.name.
        from claude_island.ui.expanded_window import SessionDetailPopup
        s = _session(1, "/proj")
        details = _make_full_details(
        ...
        qtbot.addWidget(popup)
        assert self._find_subtitle_label(
            popup, "claude md prompt",
        ) is not None

    def test_subtitle_suppressed_when_original_equals_title(self, qtbot):
        # No rename → original_name == name == title. Don't echo the
        # same string twice in the header. Verify by counting labels:
        # exactly one (the title), not two (title + subtitle).
        from PySide6.QtWidgets import QLabel
        from claude_island.ui.expanded_window import SessionDetailPopup
        s = _session(1, "/proj")
        details = _make_full_details(
        ...
        qtbot.addWidget(popup)
        matches = [
            lbl for lbl in popup.findChildren(QLabel)
            if lbl.text() == "cc-learning"
        ]
        assert len(matches) == 1, (
            f"Expected only the title label; got {len(matches)} labels"
        )

    def test_ai_title_preferred_over_original_when_both_present(self, qtbot):
        # When both candidates differ from the title, ai_title wins —
        # it's typically more descriptive than the auto-detected name.
        from claude_island.ui.expanded_window import SessionDetailPopup
        s = _session(1, "/proj")
        details = _make_full_details(
            s, name="custom",
        ...
        assert self._find_subtitle_label(popup, "The Original Name") is None
